...
class AccountEmailViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            message = '<html><body>'+str(_('<p>Hello,<br/><br/> Thank for your signup, to be free to use your account please you need to verifiy your email using the following link: '))+str('<a href="'+settings.API_BASE_URL+'/account/email/verify/?token='+token+'&profil='+str(user.id)+'">'+str(_('Click here'))+'</a><br/><br/>')+str(_('Best regards'))+str('</p><p style="color:#DB7D34">The Izycab Team</p></body></html>')

            print(message)
            from smtplib import SMTPAuthenticationError
            try:
                send_mail(subject=_('Izycab account verification'), message=message, from_email="IZYCAB<"+settings.DEFAULT_FROM_EMAIL+">", recipient_list=[serializer.validated_data['email']],
                          html_message=message, fail_silently=False)
                user.save()
            except SMTPAuthenticationError:
                return Response({'message': _('Izicab account activation')}, status=status.HTTP_423_LOCKED)
